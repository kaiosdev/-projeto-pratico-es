<div align="center">
  <h1 style="color: #2e7d32;">🌱 PLANO DE TRABALHO: SLOW DOWN</h1>
  <p><i>Engenharia de Software A - ICET/UFAM</i></p>


  <img src="https://img.shields.io/badge/Status-Concluído-blue?style=for-the-badge" alt="Status">

  <br><br>

  | Campo | Informação |
  |:---|:---|
  | **Nome do Projeto** | SlowDown |
  | **Codinome** | Slow |
  | **Responsável** | Evelly Rebecca |
  | **Coordenador** | Prof. Dr. Andrey Rodrigues |
</div>

---


## 1. INTRODUÇÃO

O burnout é reconhecido pela Organização Mundial da Saúde (OMS) desde 2019 como um fenômeno ocupacional resultante do estresse crônico no ambiente de trabalho que não foi gerenciado com sucesso. No Brasil, o problema atinge proporções alarmantes em múltiplos setores da economia — saúde, educação, tecnologia, indústria e agronegócio —, com milhões de trabalhadores afetados anualmente. No setor do agronegócio, por exemplo, foram registrados 421 benefícios aprovados pelo INSS em 2023 apenas por esgotamento físico e mental, evidenciando a gravidade do problema também em ambientes de trabalho com conectividade limitada. O **SlowDown** é um aplicativo voltado à promoção da saúde mental de trabalhadores de qualquer setor, focado na prevenção e identificação precoce da Síndrome de Burnout antes que ela se torne incapacitante.

### 1.1 Objeto
Evoluir um aplicativo focado na prevenção do burnout ocupacional em trabalhadores de qualquer setor profissional, através de monitoramento emocional contínuo e ferramentas de autogestão do estresse, com suporte offline para ambientes de conectividade limitada.

### 1.2 Motivação e Justificativa
A alta incidência de esgotamento emocional em diferentes setores da economia brasileira justifica o desenvolvimento de uma solução acessível e adaptável a diferentes perfis de trabalhadores, que utilize dispositivos móveis para promover o bem-estar de forma contínua. A ausência de ferramentas preventivas — focadas em antecipar o burnout antes do colapso, e não apenas em tratá-lo — representa uma lacuna concreta que o SlowDown se propõe a preencher.



## 2. INFORMAÇÕES GERAIS

### 2.1 Escopo Geral
O escopo abrange o desenvolvimento e manutenção evolutiva de um aplicativo mobile focado em monitoramento de estresse, missões diárias de autocuidado e apoio emocional, destinado a trabalhadores de qualquer setor profissional. O suporte offline garante acessibilidade em ambientes com conectividade instável ou ausente.

#### 2.1.1 Escopo Específico
* Monitoramento de humor e níveis de estresse com termómetro diário.
* Sistema de **Missões Diárias** e **XP** para incentivar a meditação e o registo de humor.
* **Sistema de Conquistas** com emblemas como "Mente Blindada".
* Chatbot interativo e mascote virtual personalizável (Tamagotchi).
* Mini games antiestresse e suporte offline para meditação e música.

#### 2.1.2 Escopo Negativo
* Não substitui diagnóstico ou tratamento clínico profissional.
* Não incluirá versão web completa (foco exclusivo em mobile).

### 2.2 Ambiente de Desenvolvimento
* **Metodologia:** Scrum com ciclos iterativos (sprints).
* **Gestão:** GitHub Projects, Notion e Figma.


## 3. METODOLOGIA E EQUIPE

### 3.1 Equipa de Projeto: Papéis e Responsabilidades

A distribuição foi estruturada para refletir as funções executadas em cada etapa do Trabalho Prático I.

| Membro | Papel | Responsabilidades Principais |
|:--- |:--- |:--- |
| **Kaio** | Scrum Master / Líder | Gerir o repositório GitHub; configurar diretórios; criar o quadro Backlog do Produto; supervisionar Dailys no Notion; e submeter as entregas no Classroom. |
| **Felipe** | Backlog Owner | Escrever 15+ histórias de utilizador; especificar critérios de aceitação para 10 histórias principais; e garantir a rastreabilidade com as personas. |
| **Marcos** | Design Thinking I | Elaborar o Golden Circle (Porquê? Como? O quê?); criar a imagem da Análise SWOT; e documentar a fase de Inspiração. |
| **Evelly** | Documentação | Redação e manutenção do Plano de Trabalho; definição de escopos; e revisão técnica de todos os arquivos Markdown. |
| **Nádia** | Gestão de Sprint | Manter o quadro Backlog do Sprint no GitHub Projects; e sincronizar tarefas individuais com os registos das Dailys no Notion. |
| **Marcelo** | Mercado e Personas | Pesquisar 3 soluções concorrentes; criar no mínimo 3 personas (técnica PATHY); e consolidar as ideias do brainstorming. |

### 3.2 Gestão de Comunicação
As **Dailys** são registadas no Notion de 3 a 4 vezes por semana por cada integrante, permitindo ao Scrum Master e à Gestora de Sprint monitorar impedimentos e o progresso das tarefas.


## 4. PLANEAMENTO FINANCEIRO ESTIMADO

### 4.1 Resumo Financeiro do Projeto (Ciclo de 4 meses)

| Item | Descrição | Valor Total (R$) |
|:--- |:--- |:--- |
| **Infraestrutura** | Licenças Figma e Servidores Nuvem. | R$ 960,00 |
| **Recursos Humanos** | Equipa de 6 integrantes (Simulação de Mercado). | R$ 72.000,00 |
| **Consumo** | Internet e Energia Elétrica. | R$ 4.080,00 |
| **Terceiros** | Identidade Visual e Testes de Usabilidade. | R$ 6.000,00 |
| **Contingência** | Margem de Segurança (10%). | R$ 8.304,00 |
| **TOTAL** | | **R$ 92.151,78** |

---

<div align="center">

  <sub>Desenvolvido para a disciplina de Engenharia de Software I - ICET/UFAM. <br /> Professor: Dr. Andrey Rodrigues</sub>

</div>
